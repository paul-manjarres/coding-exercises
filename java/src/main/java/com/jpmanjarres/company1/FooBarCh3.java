...
public class FooBarCh3 {
    ...
    public static int solution(int[] l) {
        ...
        subsets = subsets.stream()
                .filter(arr -> arr.stream().mapToInt(Integer::valueOf).sum() % 3 == 0)
                .collect(Collectors.toSet());
        // System.out.println("Divisibles por 3: "+subsets.size());
        // subsets.forEach(System.out::println);

        int maxSize = subsets.stream().mapToInt(ArrayList::size).max().orElse(0);
        Set<ArrayList<Integer>> candidates =
                subsets.stream().filter(arr -> arr.size() == maxSize).collect(Collectors.toSet());
        ...
        for (ArrayList<Integer> list : candidates) {
            ...
            maxNumber = Math.max(
                    maxNumber,
                    Integer.parseInt(list.stream().map(String::valueOf).collect(Collectors.joining(""))));
        }

        return maxNumber;
    }

    public static Set<ArrayList<Integer>> generateSubsets(
            int[] xs, int i, ArrayList<Integer> subset, Set<ArrayList<Integer>> set) {
        if (i == xs.length) {
            if (!subset.isEmpty()) {
